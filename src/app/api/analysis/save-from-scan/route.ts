...
// Dot product 6D: skin scores x product weights
function calculateScore(
  scores: SkinScores,
  weights: {
    w_acne: number; w_blackheads: number; w_clear_skin: number;
    w_dark_spots: number; w_puffy_eyes: number; w_wrinkles: number;
  }
): number {
...
function getDominantCondition(scores: SkinScores): string {
  const entries = (
    ['acne', 'blackheads', 'clear_skin', 'dark_spots', 'puffy_eyes', 'wrinkles'] as const
